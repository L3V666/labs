...
unsigned int height(subset_node *sn) {
    if (!sn) return 0;
    unsigned int hl = height(sn->left);
    unsigned int hr = height(sn->right);
...
bool insert(subset_node **sn, int k) {
    ...
    if (k < parent->key) parent->left = new subset_node(k);
    else parent->right = new subset_node(k);
    return true;
}
...
bool remove(subset_node **sn, int k) {
    ...
    if(cur->left && cur->right) {
        subset_node *succ_parent = cur;
        subset_node *succ = cur->right;
        while(succ->left) {
            succ_parent = succ;
            succ = succ->left;
        }
    ...
    subset_node *child = (cur->left) ? cur->left : cur->right;
    if (parent == nullptr) {
        *sn = child;
    } else {
...
void dfs_fill(subset_node *cur, int *arr, unsigned int &idx) {
    if (!cur) return;
    dfs_fill(cur->left, arr, idx);
    arr[idx++] = cur->key;
    dfs_fill(cur->right, arr, idx);
}
